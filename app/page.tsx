...
export default async function Home() {
  ...
  return <div className="site-grain overflow-hidden">
    <main className="page-glow relative border-b border-border">
      <div className="mx-auto grid max-w-7xl items-center gap-9 px-4 py-12 sm:min-h-[calc(100svh-4.7rem)] sm:px-6 sm:py-16 lg:grid-cols-[1.2fr_.8fr] lg:px-8 lg:py-20">
        <div><p className="eyebrow"><MapPin size={12} /> Enugu, Nigeria · Available for select work</p><h1 className="mt-5 max-w-4xl font-display text-4xl font-bold leading-[.9] tracking-[-.075em] sm:mt-7 sm:text-7xl lg:text-8xl">I turn messy ideas into <span className="text-brand">useful digital products.</span></h1><p className="mt-5 max-w-xl text-base leading-7 text-fg-muted sm:mt-7 sm:text-lg">I&apos;m Joshua Ugwu, a product-minded software engineer building platforms people can actually use — from student tools and SaaS products to client systems with real operations behind them.</p><div className="mt-7 flex flex-wrap gap-3 sm:mt-9"><Link href="/projects" className="button-primary">Explore my work <ArrowUpRight size={17} /></Link><Link href="/hire" className="button-secondary">Start a project</Link>{brand.resumeUrl && <a href={brand.resumeUrl} target="_blank" rel="noreferrer" className="focus-ring grid h-12 w-12 place-items-center rounded-xl border border-border bg-surface text-fg hover:border-brand" aria-label="Open resume"><Download size={18} /></a>}</div></div>
