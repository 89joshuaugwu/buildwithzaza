  ...
  return (
    <section className="mx-auto max-w-6xl px-4 py-20 sm:px-6">
      <ScrollReveal className="flex items-end justify-between gap-4">
        <div>
          <span className="log-eyebrow">selected-work --status=shipped</span>
      ...
      <ScrollReveal stagger className="mt-10 grid gap-6 sm:grid-cols-2">
        {projects.map((project) => (
          <RevealItem key={project.slug}>
            <Link href={`/projects/${project.slug}`} className="group block h-full [perspective:1000px]">
              <div className="relative h-full overflow-hidden rounded-2xl border border-border bg-surface p-6 transition-transform duration-300 [transform-style:preserve-3d] group-hover:-translate-y-1 group-hover:[transform:rotateX(2deg)_rotateY(-2deg)]">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {project.stack.map((tech) => (
                    <span
                      key={tech}
                      className="rounded-full border border-border px-2.5 py-1 font-mono text-[11px] text-fg-muted transition-colors group-hover:border-highlight/40 group-hover:text-highlight"
                    >
                      {tech}
                    </span>
                  ))}
